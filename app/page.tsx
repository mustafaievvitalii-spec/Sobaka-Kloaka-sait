...
function CtaButton({ href, children, variant = 'gold' }: { href?: string; children: React.ReactNode; variant?: 'gold' | 'ghost' }) {
  const className = variant === 'gold'
    ? 'bg-[#d6b36a] text-[#07111f] shadow-[0_18px_55px_rgba(214,179,106,.22)] hover:bg-[#e7c881]'
    : 'border border-white/15 bg-white/8 text-white hover:border-[#d6b36a]/50 hover:bg-white/12';
...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#06101d] text-white">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_18%_12%,rgba(214,179,106,.16),transparent_30%),radial-gradient(circle_at_82%_4%,rgba(75,134,209,.16),transparent_28%),linear-gradient(180deg,#07111f_0%,#081524_45%,#050b14_100%)]" />
      <div className="pointer-events-none fixed inset-0 bg-grid opacity-50" />

      <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/8 bg-[#06101d]/70 backdrop-blur-xl">
        <div className="section-shell flex h-18 items-center justify-between">
          <a href="#top" className="flex items-center gap-3" aria-label="DocExpert">
            <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#d6b36a] text-[#07111f]"><FileCheck2 className="h-5 w-5" /></span>
            <span className="text-lg font-semibold tracking-tight">DocExpert</span>
          </a>
          <nav className="hidden items-center gap-7 text-sm text-slate-300 lg:flex">
            <a className="premium-link" href="#services">Послуги</a><a className="premium-link" href="#pricing">Вартість</a><a className="premium-link" href="#process">Процедура</a><a className="premium-link" href="#faq">FAQ</a><a className="premium-link" href="#contacts">Контакти</a>
          </nav>
          <div className="hidden items-center gap-3 sm:flex"><CtaButton href={viberUrl} variant="ghost">Viber</CtaButton><CtaButton href={whatsappUrl}>WhatsApp</CtaButton></div>
        </div>
      </header>

      <section id="top" className="relative min-h-screen pt-28 sm:pt-36">
        <motion.div style={{ y: heroY }} className="absolute left-1/2 top-28 h-72 w-72 -translate-x-1/2 rounded-full bg-[#d6b36a]/20 blur-3xl" />
        <div className="section-shell grid items-center gap-12 pb-20 lg:grid-cols-[1.05fr_.95fr]">
          <motion.div variants={stagger} initial="hidden" animate="show" className="relative z-10">
            <motion.div variants={fadeUp} className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#d6b36a]/25 bg-[#d6b36a]/10 px-4 py-2 text-sm text-[#f1d99b]"><Sparkles className="h-4 w-4" /> Юридичний супровід для українців за кордоном</motion.div>
            <motion.h1 variants={fadeUp} className="max-w-4xl text-5xl font-semibold tracking-[-0.05em] text-white sm:text-7xl lg:text-8xl">Відновлення посвідчення водія дистанційно</motion.h1>
            <motion.p variants={fadeUp} className="mt-5 text-2xl font-medium text-[#d6b36a]">Для українців за кордоном</motion.p>
            <motion.p variants={fadeUp} className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">Допомагаємо внести посвідчення в Дію, перевипустити пластикове посвідчення та виправити помилки в базі МВС без необхідності приїжджати в Україну.</motion.p>
            ...
            <motion.div variants={stagger} className="mt-10 grid gap-3 sm:grid-cols-2">{['Офіційна процедура', 'Робота через державні реєстри', 'Супровід клієнтів по Європі', 'Договір про надання послуг'].map((item) => <motion.div variants={fadeUp} key={item} className="glass rounded-2xl px-4 py-3 text-sm text-slate-100"><Check className="mr-2 inline h-4 w-4 text-[#d6b36a]" />{item}</motion.div>)}</motion.div>
          </motion.div>
          <motion.div style={{ y: cardY }} className="relative z-10 hidden lg:block">
            <div className="glass gold-glow relative rounded-[2.5rem] p-8">
              <div className="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-[#d6b36a]/20 blur-2xl" />
              <div className="rounded-[2rem] border border-white/10 bg-[#081828]/90 p-7">
                <div className="mb-7 flex items-center justify-between"><div><p className="text-sm text-slate-400">Case review</p><h3 className="text-2xl font-semibold">DocExpert Legal Desk</h3></div><ShieldCheck className="h-10 w-10 text-[#d6b36a]" /></div>
                {['Первинний аналіз документів', 'Запит / верифікація даних', 'Оновлення в Дії', 'Доставка по Європі'].map((item, i) => <motion.div key={item} animate={{ x: [0, i % 2 ? -4 : 4, 0] }} transition={{ duration: 4 + i, repeat: Infinity }} className="mb-4 rounded-2xl border border-white/10 bg-white/[0.045] p-4"><span className="text-xs text-[#d6b36a]">0{i + 1}</span><p className="mt-1 text-slate-100">{item}</p></motion.div>)}
              </div>
            </div>
          </motion.div>
        </div>
